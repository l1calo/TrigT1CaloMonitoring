...
/*---------------------------------------------------------*/
{
  ...
  MonGroup BS_CPM ( this, "L1CaloStats/BS_CPM", expert, eventsBlock );

  MonGroup Sim_CPM ( this, "L1CaloStats/Sim_CPM", expert, eventsBlock );

  if ( isNewEventsBlock || isNewLumiBlock ) {	
  
  //  CPM Tower - Trigger Tower comparison Histos

  m_h_TT_Em_Et = book1D(BS_CPM,"TT_EM_Et","Trigger Tower EM Et Noise",10,0,10);
  m_h_TT_Had_Et = book1D(BS_CPM,"TT_HAD_Et","Trigger Tower HAD Et Noise",10,0,10);
  m_h_CT_Em_Et = book1D(BS_CPM,"CT_EM_Et","CPM Tower EM Et Noise",10,0,10);
  m_h_CT_Had_Et = book1D(BS_CPM,"CT_HAD_Et","CPM Tower HAD Et Noise",10,0,10);
  m_h_TT_Em_Et_s = book1D(BS_CPM,"TT_EM_Et_s","Trigger Tower EM Et Signal",245,10,255);
  m_h_TT_Had_Et_s = book1D(BS_CPM,"TT_HAD_Et_s","Trigger Tower HAD Et Signal",245,10,255);
  m_h_CT_Em_Et_s = book1D(BS_CPM,"CT_EM_Et_s","CPM Tower EM Et Signal",245,10,255);
  m_h_CT_Had_Et_s = book1D(BS_CPM,"CT_HAD_Et_s","CPM Tower HAD Et Signal",245,10,255);
  m_h_TT_Em_eta = book1D(BS_CPM,"TT_EM_eta","Trigger Tower EM eta",50,-2.5,2.5);
  m_h_TT_Had_eta = book1D(BS_CPM,"TT_HAD_eta","Trigger Tower HAD eta",50,-2.5,2.5);
  m_h_CT_Em_eta = book1D(BS_CPM,"CT_EM_eta","CPM Tower EM eta",50,-2.5,2.5);
  m_h_CT_Had_eta = book1D(BS_CPM,"CT_HAD_eta","CPM Tower HAD eta",50,-2.5,2.5);
  m_h_TT_Em_phi = book1D(BS_CPM,"TT_EM_phi","Trigger Tower EM phi ",64,0,2*M_PI);
  m_h_TT_Had_phi = book1D(BS_CPM,"TT_HAD_phi","Trigger Tower HAD phi ",64,0,2*M_PI);
  m_h_CT_Em_phi = book1D(BS_CPM,"CT_EM_phi","CPM Tower EM phi ",64,0,2*M_PI);
  m_h_CT_Had_phi = book1D(BS_CPM,"CT_HAD_phi","CPM Tower HAD phi ",64,0,2*M_PI);
  m_h_TT_Em_eta_phi = book2D(BS_CPM,"TT_EM_eta_phi","Trigger Tower EM eta/phi",
                                                50,-2.5,2.5,64,0,2*M_PI);
  m_h_TT_Had_eta_phi = book2D(BS_CPM,"TT_HAD_eta_phi","Trigger Tower HAD eta/phi",
                                                50,-2.5,2.5,64,0,2*M_PI);
  m_h_CT_Em_eta_phi = book2D(BS_CPM,"CT_EM_eta_phi","CPM Tower EM eta/phi",
                                                50,-2.5,2.5,64,0,2*M_PI);
  m_h_CT_Had_eta_phi = book2D(BS_CPM,"CT_HAD_eta_phi","CPM Tower HAD eta/phi",
                                                50,-2.5,2.5,64,0,2*M_PI);
  m_h_TT_Em_eta_phi_w = book2D(BS_CPM,"TT_EM_eta_phi_w",
               "Trigger Tower EM eta/phi weighted", 50,-2.5,2.5,64,0,2*M_PI);
  m_h_TT_Had_eta_phi_w = book2D(BS_CPM,"TT_HAD_eta_phi_w",
               "Trigger Tower HAD eta/phi weighted", 50,-2.5,2.5,64,0,2*M_PI);
  m_h_CT_Em_eta_phi_w = book2D(BS_CPM,"CT_EM_eta_phi_w",
                    "CPM Tower EM eta/phi weighted", 50,-2.5,2.5,64,0,2*M_PI);
  m_h_CT_Had_eta_phi_w = book2D(BS_CPM,"CT_HAD_eta_phi_w",
  ...
  for (int thresh = 0; thresh < 16; ++thresh) {
    ...
    m_v_thresholds.push_back(book1D(BS_CPM, name, title, 56, 0, 56));
  }
  ...
  for (int thresh = 0; thresh < 16; ++thresh) {
    ...
    m_v_CMM_thresholds.push_back(book1D(BS_CPM, name, title, 56, 0, 56));
  }
  for (int thresh = 0; thresh < 16; ++thresh) {
    std::ostringstream cnum;
    cnum << thresh;
    ...
    m_v_CMM_T_thresholds.push_back(book1D(BS_CPM, name, title, 20, 0, 20));
  }
  ...
  for (int thresh = 0; thresh < 16; ++thresh) {
    ...
    m_v_RoI_thresholds.push_back(book1D(BS_CPM, name, title, 56, 0, 56));
  }
  ...
  for (int thresh = 0; thresh < 16; ++thresh) {
    ...
    m_v_RoI_2D_thresholds.push_back(book2D(BS_CPM, name, title, 51, -2.55, 2.55,
...
/*---------------------------------------------------------*/
{
  ...
  TriggerTowerCollection::const_iterator ttIterator = triggerTowerTES->begin(); 
  TriggerTowerCollection::const_iterator ttIteratorEnd = triggerTowerTES->end(); 
...
TH1D* TrigT1CaloCpmMonTool::book1D(MonGroup& mg, 
                                   std::string nam, std::string tit,
                                   int nx, double xmin, double xmax)
{
  TH1D *hist = new TH1D(TString(nam), TString(tit), nx, xmin, xmax);
  
  if (mg.regHist(hist) != StatusCode::SUCCESS) {
    MsgStream log(msgSvc(), name());
...
TH2D* TrigT1CaloCpmMonTool::book2D(MonGroup& mg,
                                   std::string nam, std::string tit,
                                   int nx, double xmin, double xmax,  
	                           int ny, double ymin, double ymax)
{		
  TH2D *hist = new TH2D(TString(nam), TString(tit), nx, xmin, xmax,
                                                    ny, ymin, ymax);
